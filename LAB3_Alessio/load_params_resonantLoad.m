...
rpms2radss = 2*pi/60;           % [rpm/s] − > [rad/s^2]
...
%ppr = 500*4; % all motor ecpest to MOTORE 8 e MOTORE 10
%ppr = 1024*4; % motor 10 or 8
%pulse2deg = 360/ppr
...
sens.pot2.offset = 2.544;                                     % to subtract to uPot2_meas of the BlackBox [V]
...
%% Approximate plant parameters
Req = mot.R + sens.curr.Rs;
Jeq = mot.J + mld.Jh/gbox.N^2;
Beq = 1.067548701517996e-06;         % estimated in the laboratory activity 0
tau_sf = 0.012480599355650;          % estimated in the laboratory activity 0
 
km = mot.Kt*drv.dcgain/(Req*Beq + mot.Kt*mot.Ke);
Tm = Req*Jeq/(Req*Beq + mot.Kt*mot.Ke);
den_B = [mld.Jb, mld.Bb];            % denominator of the beam dynamics TF