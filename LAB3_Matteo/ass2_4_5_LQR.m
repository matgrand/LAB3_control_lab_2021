...
%LQR parameters
rho = 50; %balance between input and state (rho small -> input very costly)
...
sysG_1e = ss(A_1e, B_1e, C_1e, D_1e);

%using Bryson's rule
Q = diag([q11 1/dev_thh^2 1/dev_thd^2 0 0]);
R = rho/dev_u^2;

K_ss_1e = lqr(sysG_1e, Q, R);
K_ss_e = K_ss_1e*Te; %switch back to original system
...
%8 validation 
%INTEGRAL ACTION LQR 
enable_integral_action = 1; %disable integral action
step_height = 50; %deg
